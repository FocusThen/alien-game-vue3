  ...
  <div class="contain">
    ...
    <section v-else-if="uiState === 'characterChosen'">
      ...
      <div class="zombietalk">
        <p v-for="person in shuffledCharacterChoices" :key="person">
          <button @click="pickAnswer(person)">
            {{ questions[questionIndex][person] }}
          </button>
        </p>
      </div>
    </section>
...
export default {
  ...
  setup() {
    const {
      ...
      character,
      /**
       * Actually you don't need mutations at all
       * you can change value like score.value += 1 🤘
       */
      mutations,
    } = GameStore
    ...
    const pickCharacter = () => {
      /**
       * character.value = characterInput.value
       * uiState.value = 'characterChosen'
       */

      mutations.pickCharacter(characterInput.value)
      mutations.updateUIState('characterChosen')
    }

    const pickAnswer = (person) => {
      /**
       *  questionIndex.value++
       */
      mutations.pickAnswer(person)
    }

    const shuffledCharacterChoices = computed(() =>
      shuffle(characterChoices.value)
    )
